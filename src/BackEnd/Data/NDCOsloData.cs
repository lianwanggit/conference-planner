    ...
    public static class NDCOsloData
    {
        ...
        public static void Seed(IServiceProvider services)
        {
            ...
            using (var scope = services.CreateScope())
            {
                ...
                // Conference
                var localTime = DateTime.Now.Date.AddDays(3);
                localTime = DateTime.SpecifyKind(localTime, DateTimeKind.Unspecified);
                var conference = new Conference
                {
                    Name = "Anaheim 2017", 
                    StartDate = new DateTimeOffset(localTime, TimeSpan.FromHours(1))
                };
                conference.Slug = conference.Name.ToSlug();
                conference.EndDate = conference.StartDate?.AddDays(3);
                db.Conferences.Add(conference);

                conference = new Conference {
                    Name = "NDC Oslo 2017",
                    StartDate = new DateTimeOffset(2017, 6, 14, 0, 0, 0, TimeSpan.FromHours(1))
                };
                conference.Slug = conference.Name.ToSlug();
                conference.EndDate = conference.StartDate?.AddDays(2);
                db.Conferences.Add(conference);
